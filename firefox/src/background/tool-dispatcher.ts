...
export class ToolDispatcher {
  ...
  private async screenshot(tab: browser.tabs.Tab, args: Record<string, unknown> | undefined): Promise<unknown> {
    ...
    try {
      const dataUrl = await browser.tabs.captureVisibleTab(tab.windowId as number, {
        format,
        quality,
      } as { format: "png" | "jpeg"; quality?: number });
      ...
      if (/visible|active|not visible/i.test(message)) {
        throw new PiBrowserProtocolError(
          ...
          `cannot capture tab ${tab.id}: it must be the visible tab in its window`,
        );
      }
