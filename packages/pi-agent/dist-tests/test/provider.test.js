...
import { BROWSER_TOOLS, CLIENT_METHODS, PI_BROWSER_ERROR, PiBrowserProtocolError, X_PI_BROWSER, toErrorObject, } from "@pi-browser/protocol";
const quiet = createLogger({ level: "error", stderr: { write: () => true } });
function setupFakeFirefox() {
...
test("selectMode: legacy by default, mcp-acp when client declares acp server", () => {
    ...
    assert.equal(provider.selectMode([{ type: "stdio", name: "x" }]), "legacy");
...
test("legacy transport: tool call round-trip over x-pi-browser/tool", async () => {
    ...
    assert.ok(result.content[0].text.includes("Salvage Rush"));
    assert.ok(result.details.piBrowser);
});
test("legacy transport: structured errors from Firefox map to PiBrowser error codes", async () => {
    ...
    await assert.rejects(click.execute("tc", { ref: "el-1" }, undefined), (err) => err instanceof PiBrowserProtocolError && err.code === PI_BROWSER_ERROR.BROWSER_TAB_CLOSED);
});
test("translateTransportError maps transport errors to structured codes", async () => {
    ...
    const timeout = translateTransportError(new TransportTimeoutError("x-pi-browser/tool", 40), "browser_get_page");
    ...
    const structured = translateTransportError({ jsonrpc: "2.0", code: -32010, message: "tab gone", data: { piBrowserError: "BROWSER_TAB_CLOSED" } }, "browser_click");
...
test("legacy transport: real timeout over a non-responding peer", async () => {
    ...
    const err = (await transport.call("s1", "browser_get_page", {}).catch((e) => e));
...
test("mcp-acp transport: full handshake + tools/call over mcp/message", async () => {
    ...
    const transport = new NativeMcpOverAcpTransport(ff.b.transport, "browser-provider-123", quiet, "session-x");
    const tools = await transport.listTools();
    assert.equal(tools.length, BROWSER_TOOLS.length);
    assert.ok(tools.every((t) => t.name.startsWith("browser_")));
    // Handshake order observed by the fake server.
    assert.deepEqual(ff.mcpSeen.map((m) => m.method).slice(0, 3), ["initialize", "notifications/initialized", "tools/list"]);
...
test("mcp-acp transport: unknown serverId surfaces as MCP_UNAVAILABLE", async () => {
    ...
    await assert.rejects(transport.call("s", "browser_get_page", {}), (err) => err instanceof PiBrowserProtocolError && err.code === PI_BROWSER_ERROR.MCP_UNAVAILABLE);
});
test("provider: mcp-acp session routes tool execution through the MCP channel", async () => {
    ...
    const getPage = tools.find((t) => t.name === "browser_get_page");
    await assert.rejects(getPage.execute("tc", {}, undefined), 
