...
test("multiple frames in one read", () => {
    ...
    assert.deepEqual(out.map((f) => JSON.parse(f.toString("utf8")).id), [1, 2, 3]);
});
test("frame split across multiple reads", () => {
    ...
    const out = decoder.readAll();
    assert.equal(out.length, 1);
    assert.equal(JSON.parse(out[0].toString("utf8")).id, 2);
});
...
test("large frame (screenshot-sized) works", () => {
    ...
    for (let i = 0; i < frame.length; i += 65536)
        decoder.push(frame.subarray(i, i + 65536));
    const out = decoder.readAll();
    assert.equal(out.length, 1);
    assert.equal(JSON.parse(out[0].toString("utf8")).data.length, payload.length);
});
