...
export function defaultPkgRoot() {
    ...
    switch (command) {
        ...
        case "status": {
            ...
            if (report.installed) {
                ...
                if (addon.fresh)
                    ...
                    note(addon.detected ? "status: HOST OK — add-on heartbeat stale (reload the add-on or check Firefox)" : "status: HOST OK — add-on not detected yet (load the add-on)");
            }
            else {
        ...
        case "doctor": {
            ...
            if (addon.fresh)
                ...
                note(addon.detected ? "doctor: HOST OK — add-on heartbeat stale (reload the add-on or check Firefox)" : "doctor: HOST OK — next: load the add-on (it auto-connects within ~10s)");
            return { ok: true, lines };
        }
        case "uninstall": {
 ...
 */
export async function probeHost(env, status, timeoutMs = 20_000) {
    const lines = [];
    ...
    return new Promise((resolve) => {
        ...
        child.on("close", (code) => {
            if (!gotPong) {
                ...
                if (stderrTail)
                    lines.push(`probe stderr: ${stderrTail.trim().split("\n").slice(-3).join(" | ")}`);
                finish(false);
            }
        });
