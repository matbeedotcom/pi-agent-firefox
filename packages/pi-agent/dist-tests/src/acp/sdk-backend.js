        ...
        const session = {
            ...
            prompt: async (text, images) => {
                ...
                const opts = images && images.length > 0
                    ? { images: images.map((i) => ({ type: "image", source: { type: "base64", mediaType: i.mimeType, data: i.data } })) }
                    : undefined;
                try {
                    await agentSession.prompt(text, opts);
                }
                catch (err) {
                    // A prompt that is rejected outright (not an aborted turn) is a
                    // protocol-level failure the client should see.
                    throw new PiBrowserProtocolError(PI_BROWSER_ERROR.INTERNAL, `prompt failed: ${err instanceof Error ? err.message : String(err)}`);
                }
            ...
            subscribe: (listener) => {
                return agentSession.subscribe((event) => {
                    for (const mapped of mapBackendEvent(event))
                        listener(mapped);
                });
            },
            getHistory: async () => {
