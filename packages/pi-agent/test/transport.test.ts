...
import { JSONRPC_ERROR, type JsonRpcErrorObject } from "@pi-browser/protocol";

const quiet = createLogger({ level: "error", stderr: { write: () => true } });

function pair(): { a: Dispatcher; b: Dispatcher } {
...
test("error responses carry structured error objects", async () => {
  ...
  const err = (await a.transport.request("m").catch((e) => e)) as JsonRpcErrorObject;
  assert.equal(err.code, JSONRPC_ERROR.INVALID_PARAMS);
  assert.equal((err.data as { piBrowserError: string }).piBrowserError, "SESSION_BUSY");
});
...
test("request timeout rejects with TransportTimeoutError", async () => {
  ...
  await assert.rejects(
    a.transport.request("slow", {}, 30),
    (err: unknown) => err instanceof TransportTimeoutError && err.method === "slow",
  );
});
...
test("unknown method gets METHOD_NOT_FOUND", async () => {
  ...
  const err = (await a.transport.request("nope").catch((e) => e)) as JsonRpcErrorObject;
